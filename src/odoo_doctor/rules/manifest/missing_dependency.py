...
def check_missing_dependency(ctx: ModuleContext) -> list[Diagnostic]:
    ...
    for model_info in ctx.models.values():
        for inherited in model_info.inherit:
            ...
            if lookup.status == ResolveResult.FOUND:
                ...
                if owner_mod and owner_mod != ctx.name and owner_mod not in depends_set and owner_mod not in _ALWAYS_AVAILABLE:
                    missing_deps.setdefault(owner_mod, []).append(f"Model inherit: '{inherited}'")

    # 2. Check XML ID refs
    for rec in ctx.xml_records:
        for ref in rec.refs:
            mod = _module_from_external_id(ref)
            if mod and mod != ctx.name and mod not in depends_set and mod not in _ALWAYS_AVAILABLE:
                missing_deps.setdefault(mod, []).append(f"XML ref in record '{rec.xml_id}': '{ref}'")
    ...
    for view in ctx.views:
        if view.inherit_id:
            ...
            if mod and mod != ctx.name and mod not in depends_set and mod not in _ALWAYS_AVAILABLE:
                missing_deps.setdefault(mod, []).append(f"Inherited view in view '{view.xml_id}': '{view.inherit_id}'")
