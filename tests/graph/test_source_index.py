...
def test_build_source_index(tmp_path: Path):
    ...
    models_dir.mkdir()
    (models_dir / "sale.py").write_text(dedent("""\
    ...
    (mock_addon / "data.xml").write_text(dedent("""\
        <odoo>
...
def test_build_source_index_from_direct_addons_root(tmp_path: Path):
    ...
    models_dir = mock_addon / "models"
    models_dir.mkdir()
    (models_dir / "direct.py").write_text(dedent("""\
        from odoo import models
...
def test_build_source_index_from_odoo_addons_layout(tmp_path: Path):
    ...
    models_dir = mock_addon / "models"
    models_dir.mkdir()
    (models_dir / "core.py").write_text(dedent("""\
        from odoo import models
...
def test_resolver_with_source_index(tmp_path: Path):
    mock_addon = tmp_path / "addons" / "mock_sale"
    mock_addon.mkdir(parents=True)
    (mock_addon / "__manifest__.py").write_text('{"name": "Mock Sale", "data": ["data.xml"]}')
    
    models_dir = mock_addon / "models"
    models_dir.mkdir()
    (models_dir / "sale.py").write_text('class X(models.Model):\n    _name = "mock.model"')
    
    (mock_addon / "data.xml").write_text('<odoo><record id="mock_rec" model="mock.model"/></odoo>')
